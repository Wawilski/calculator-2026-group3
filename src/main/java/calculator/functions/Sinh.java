...
public final class Sinh extends UnaryFunction {
  ...
  public BaseNumber function(IntegerNumber value) {
    double angle = (ExpressionParser.isAngleUnitDegree) ? Math.toRadians(value.getValue())
        : value.getValue();
    return new RealNumber(Math.sinh(angle));
  }
  ...
  public BaseNumber function(ComplexNumber value) {
    ...
    if (value.isNaN()) {
      ...
      double resultReal = Math.sinh(real) * Math.cos(imaginary);
      double resultImaginary = Math.cosh(real) * Math.sin(imaginary);

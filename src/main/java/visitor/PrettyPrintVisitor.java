...
public class PrettyPrintVisitor extends Visitor {
    ...
    /**
     *  RenderedNode is a helper class to keep track of the rendered text of a sub-expression,a
     */
    private static final class RenderedNode {
        private final String text; // the rendered text of the sub-expression
        private final int precedence; // the precedence of the top-level operator in the sub-expression (0 for numbers, 1 for + and -, 2 for * and /)
    ...
    private final Deque<RenderedNode> renderedExpressions = new ArrayDeque<>();// stack to keep track of the rendered sub-expressions
    ...
    public void visit(Operation o) {
        ...
        String text = renderOperation(symbol, precedence, args);// render the operation with the appropriate parentheses based on the precedence of the sub-expressions
     ...
     * @return The precedence of the operator (e.g. 1 for + and -, 2 for * and /, 0 for unknown operators)
     */
    ...
    /**
     * Render an operation with the appropriate parentheses based on the precedence of its arguments
     * @param symbol The symbol of the operator (e.g. "+", "*")
     * @param precedence The precedence of the operator (e.g. 1 for + and -, 2 for * and /)
    ...
    /** Determine whether to parenthesize a child expression based on its precedence relative to the parent operator
     * @param parentSymbol The symbol of the parent operator (e.g. "+", "*")
     * @param parentPrecedence The precedence of the parent operator (e.g. 1 for + and -, 2 for * and /)
     * @param child The rendered node of the child expression
     * @param argIndex The index of the child argument in the parent's argument list (0-based)
     * @return The rendered text of the child expression, with parentheses if necessary
     */

...
class TestCalculatorApiController {
    ...
    @Test
    void testEvaluateEndpointRejectsBlankType() throws Exception {
        String payload = """
                {
                  "type": "   ",
                  "value": 4
        ...
        mockMvc.perform(post("/api/evaluate")
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("VALIDATION_ERROR"))
                .andExpect(jsonPath("$.details", hasItem("type: type must not be blank")));
    }

    @Test
    void testEvaluateEndpointRejectsUnexpectedFields() throws Exception {
        String payload = """
                {
                  "type": "number",
                  "value": 7,
        ...
        mockMvc.perform(post("/api/evaluate")
                ...
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.code").value("MALFORMED_JSON"))
                .andExpect(jsonPath("$.details[0]", startsWith("Unrecognized property \"unexpected\"")));
    }

    @Test
    void testEvaluateEndpointRejectsTooLargePayload() throws Exception {
        String payload = """
                {
                  "type": "plus",
                  "args": [
